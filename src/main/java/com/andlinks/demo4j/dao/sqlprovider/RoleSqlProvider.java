...
/**
 * Created by 王凯斌 on 2017/3/2. role的SqlProvider类
 */
public class RoleSqlProvider extends BaseSqlProvider<RoleDO> {

	public String removeUsers() {
		return new SQL().DELETE_FROM("user_role").WHERE("role_uuid=#{uuid}").toString();
	}
	...
	public String removePermissions() {
		return new SQL().DELETE_FROM("role_permission").WHERE("role_uuid=#{uuid}").toString();
	}
	...
	public String listUsers(String uuid) {
		return new SQL().SELECT("u.id,u.uuid,u.user_name,u.password").FROM("user u")
				.LEFT_OUTER_JOIN("user_role ur on u.uuid = ur.user_uuid").WHERE("ur.role_uuid = #{uuid}", "deleted = 0")
				.toString();
	}

	public String listPermissions() {
		return new SQL().SELECT("p.id,p.uuid,p.permission_name").FROM("permission p")
				.LEFT_OUTER_JOIN("role_permission rp on p.uuid = rp.permission_uuid")
				.WHERE("rp.role_uuid = #{uuid}", "deleted = 0").toString();
	}
}
